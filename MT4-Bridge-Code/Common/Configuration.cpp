...
//|                                                                  |
//+------------------------------------------------------------------+
int CConfiguration::SortIndex(const void *left,const void *right)
  {
  return((*((PluginCfg **)left))->reserved[0]-(*((PluginCfg **)right))->reserved[0]);
  }
//+------------------------------------------------------------------+
//| Unblockable Name Search                                          |
//+------------------------------------------------------------------+
PluginCfg *CConfiguration::Search(LPCSTR name)
  {
  PluginCfg *config=NULL;
  //----
  if(m_cfgs!=NULL&&m_cfgs_total>0)
    config=(PluginCfg *)bsearch(name,m_cfgs,m_cfgs_total,sizeof(PluginCfg),SearchByName);
  //----
  return(config);
  }
//+------------------------------------------------------------------+
...
CConfiguration::CConfiguration(): m_cfgs(NULL),m_cfgs_total(0),m_cfgs_max(0),m_cfgs_index(NULL)
  {
  StringType::Copy(m_cfgname,"MT4.Bridge.ini");
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
void CConfiguration::Load()
  ...
  //---- store filename
  path.Format("%s\\%s",ExtProgramPath,m_cfgname);
  LOG_INFO(pluginLog,"loading parameters [path:%s]",path.Str());
  //---- open file
  if(!file.Open(path.Str(),GENERIC_READ,OPEN_ALWAYS))
    {
    m_sync.Unlock();
    return;
    }
  ...
  while(file.GetNextLine(buffer,sizeof(buffer)-1)>0)
    ...
    *cp=0;
    //---- get parameter name
    StringType::TrimSpaces(start);
    StringType::Copy(cfg.name,start);
    //---- skip whitspaces
    start=cp+1; while(*start==' ') start++;
    //---- receive parameter value
    StringType::Copy(cfg.value,start);
    //---- set order
    ...
    if(m_cfgs==NULL||m_cfgs_total>=m_cfgs_max)
      {
      //---- allocate new buffer
      if((temp=new PluginCfg[m_cfgs_total+1024])==NULL) { m_sync.Unlock(); return; }
...
int CConfiguration::Save(void)
  ...
  StringTypePath path;
  //---- checks
  if(m_cfgs==NULL||m_cfgs_total<0) return(FALSE);
  ...
  //---- opening file
  path.Format("%s\\%s",ExtProgramPath,m_cfgname);
  if(!file.Open(path.Str(),GENERIC_WRITE,CREATE_ALWAYS)) return(FALSE);
  //---- writing parameters
  for(i=0;i<m_cfgs_total;i++)
    ...
    //---- store to disk
    len=file.Write(buffer,strlen(buffer));
    }
  //---- close file
  file.Close();
  //---- return ok
...
int CConfiguration::GetInteger(const int pos,const char *name,int *value,const char *defvalue)
  ...
  if(Get(name,&cfg,pos)==FALSE)
    ...
    StringType::Copy(cfg.name,name);
    if(defvalue!=NULL) StringType::Copy(cfg.value,defvalue);
    //---- add
...
//| Get float by name                                                |
//+------------------------------------------------------------------+
int CConfiguration::GetBool(const int pos  ,const char *name,bool *value,const char *defvalue)
  {
  PluginCfg cfg={0};
  //---- checks
  if(name==NULL||value==NULL) return(FALSE);
