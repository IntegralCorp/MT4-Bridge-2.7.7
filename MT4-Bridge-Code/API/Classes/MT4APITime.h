...
private:
   ...
   static const LPCSTR s_months_names[13];
   static const LPCSTR s_months_short_names[13];

public:
   //--- time functions
   static bool       ParseTime(const INT64 ctm,tm *ttm);
   static INT64      MakeTime(tm *ttm);
   static LPCSTR     MonthName(const UCHAR month);
   static LPCSTR     MonthNameShort(const UCHAR month);
   static INT64      WeekBegin(const INT64 ctm);
   static INT64      DayBegin(const INT64 ctm);
   static INT64      HourBegin(const INT64 ctm);
...
//|                                                                  |
//+------------------------------------------------------------------+
const __declspec(selectany)LPCSTR  SMTTime::s_months_names[13]      ={ "January","February","March","April","May","June","July","August","September","October","November","December","Unknown" };
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
const __declspec(selectany)LPCSTR  SMTTime::s_months_short_names[13]={ "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Unk" };
//+------------------------------------------------------------------+
//| Time parsing                                                     |
//+------------------------------------------------------------------+
...
//| Month full name                                                  |
//+------------------------------------------------------------------+
inline LPCSTR SMTTime::MonthName(const UCHAR month)
  {
...
//| Month short name                                                 |
//+------------------------------------------------------------------+
inline LPCSTR SMTTime::MonthNameShort(const UCHAR month)
  {
...
//+------------------------------------------------------------------+
//| Hour begin calculation                                           |
//+------------------------------------------------------------------+
inline INT64 SMTTime::HourBegin(const INT64 ctm)
  {
   return((ctm/SECONDS_IN_HOUR)*SECONDS_IN_HOUR);
  }
//+------------------------------------------------------------------+
