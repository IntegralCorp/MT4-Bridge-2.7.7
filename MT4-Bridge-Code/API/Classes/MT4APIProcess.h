...
public:
   ...
   bool             Start(StringType &command,const DWORD flags=DETACHED_PROCESS,const bool inherit=false);
   void             Close(void);
...
//| Process start                                                    |
//+------------------------------------------------------------------+
inline bool CMTProcess::Start(StringType &command,const DWORD flags/*=CREATE_NO_WINDOW*/,const bool inherit/*=false*/)
  {
   STARTUPINFO          si={0};
...
//--- create process
   if(CreateProcessA(NULL,command.Buffer(),NULL,NULL,inherit ? TRUE:FALSE,flags,NULL,NULL,&si,&pi)==FALSE)
      return(false);
//--- 
