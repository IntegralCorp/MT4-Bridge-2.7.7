...
//--- check
   if(!path || !zip_path)
      return(false);
//--- get file attributes
   if(GetFileAttributesEx(path,GetFileExInfoStandard,&fa)==FALSE)
      return(FALSE);
//--- open file
...
//---
   ...
   if((hFind=FindFirstFile(tmp.Str(),&ffd))==INVALID_HANDLE_VALUE)
      return(true);
//--- iterate directory
   do
     {
         ...
         if(wcscmp(mask,L"*.*")==0)
           {
            ...
            if(zip_path[0]!=0)
               ...
               tmp_zip.Format(L"%s",ffd.cFileName);
            //--- add directory
...
void CZipFile::UpdateZipInfo(ZipFileInfo &info)
  {
   ...
   SYSTEMTIME                st;
//--- get file attributes
   if(GetFileAttributesEx(info.file_path,GetFileExInfoStandard,&fa)==FALSE)
      return;
//---
...
//--- update
   info.tmzDate    =st;
   info.external_fa=GetFileAttributes(info.file_path);
  }
//+------------------------------------------------------------------+
//| Open file in ZIP                                                 |
//+------------------------------------------------------------------+
